...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            nargs='+',
            type=int,
            help='Specific store IDs to scrape (overrides file if given)'
        )
        parser.add_argument(
            '--sync',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        # ✅ If no --stores given, load from store_ids.txt
        if not store_ids:
            store_file = Path(__file__).resolve().parent.parent.parent / "store_ids.txt"
            if store_file.exists():
                ...
                self.stdout.write(f"Loaded {len(store_ids)} store IDs from {store_file}")
            else:
                self.stdout.write(
                    self.style.ERROR(f"store_ids.txt not found at {store_file}")
                )
                return

        self.stdout.write(f'Starting scraping for {target_date}')
        self.stdout.write(f'Targeting {len(store_ids)} stores')
